class_name WorkstationUI extends Control
...
func _ready():
	...
	# Add title
	var title = Label.new()
	title.text = "Workstation Components"
	title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	title.add_theme_font_size_override("font_size", 20)
	component_container.add_child(title)
	
	# Load components from global
	var components = GlobalFileWorkstation.load_workstation_data()
...
func _on_component_button_pressed(component_name):
	...
	var workstation_builder = get_node("/root/Main/BuildingBuilder")  # Adjust path as needed
	if workstation_builder:
		workstation_builder.update_size_options(component_name)
