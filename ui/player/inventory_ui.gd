...
func add_item_entry(item):
	...
	button.text = "%s - %d" % [item.game_object_id, item.quantity]
...
func _on_item_button_pressed(item):
	pass
	print("Selected item: ", item.game_object_id)
	if item.equipable_item_game_object:
		item.equipable_item_game_object.equip(GlobalPlayer.player)
