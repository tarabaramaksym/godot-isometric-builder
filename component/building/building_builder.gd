		...
		for option_set in component_data.size_options:
			...
			for axis in ["x", "y", "z"]:
				if axis in option_set:
					...
					current_size_options[axis] = {"min": axis_data.min, "max": axis_data.max}
...
func set_size_value(axis, value):
	if (
		axis in current_size_options
		and value >= current_size_options[axis].min
		and value <= current_size_options[axis].max
	):
...
func update_preview_position(param_position: Vector3):
	if preview_instance:
		if GlobalBuilding.selected_component.is_empty() or not is_valid_component():
			return
		...
		position_handler.update_preview_position(preview_instance, param_position, component_data)
...
func add_mesh(bodyPosition: Vector3, parent_node: Node, _is_plane = true):
	...
	game_object.initialize_game_object(
		component_name,
		{
			...
			"current_size_values": current_size_values
		}
	)
	...
	if component_data.mesh_type == "simple":
		...
		create_array_mesh(game_object, component_data)
...
func create_preview_simple_mesh(
	root_node: Node3D, component_data: Dictionary, is_drag_preview: bool
):
	...
	# Create a material with transparency for preview
	var material = StandardMaterial3D.new()
	material.albedo_color = Color(0.8, 0.8, 0.8, 0.5)
	material.flags_transparent = true

	if "rgb_color" in component_data:
		var preview_alpha = 0.5  # Keep preview transparency
	...
	position_handler.apply_simple_mesh_position(
		root_node, meshChild.size, component_data, is_drag_preview
	)
	...
	for mesh_data in mesh_array:
		...
		if mesh_data.mesh == "BoxMesh3D":
			...
			if "base_mesh_size" in mesh_data:
				...
				meshChild.size = Vector3(
					current_size_values.get("x", base_size.x), base_size.y, base_size.z
				)
...
func add_collision_shape(root_node: Node3D, size: Vector3, param_position: Vector3):
	var collision_shape = CollisionShape3D.new()
	var shape = BoxShape3D.new()
	...
	collision_shape.transform.origin = param_position
